...
class mapObj(basemap.Basemap):
  ...
  **Example**:
  ...
  def __init__(self, datetime=None, coords='geo', 
    ...
    fillContinents='.8', fillOceans='b', fillLakes=None, coastLineWidth=0., 
    lineColor = .75,grid=True, gridLabels=True, showCoords=True, **kwargs):
    """Create empty map 
    ...
    **Example**:
    ...
    if grid:
      ...
      out = self.drawparallels(parallels, color=lineColor, zorder=10)
      ...
      # draw meridians
      out = self.drawmeridians(meridians, labels=merLabels, color=lineColor, zorder=10)
